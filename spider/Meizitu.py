...
from db import db_session,ErrorPage,ErrDLPic
def getPicUrls(num):
...
def downloadImageFile(imgUrl):
    ...
    with open("C:\\Users\\fxre\\Downloads\\meizi\\"+local_filename, 'wb') as f:
...
def main():
    for i in range(1, 50000, 1):
        ...
        picUrl = getPicUrls(i)
        if picUrl:
            db_session.merge(ErrorPage(i))
        if downloadImageFile(picUrl) is not None:
            print "下载成功\n"
        else:
            print "下载失败\n"
            db_session.merge(ErrDLPic(picUrl))
